...
class DataBase{
	...
	public function insert($table_name, $new_values){
		...
		$query .= ") VALUES (";
		foreach ($new_values as $field => $value) $query .= "'".addslashes($value)."',";
		$query = substr($query, 0, -1);
		$query .= ")";
	...
	public function getRecordByFieldValue($table_name, $field, $value){//добавил 16.02.19 - должно выдавать первую найденную запись. Для уник. элем. она и единственная
	...
	public function deleteOnID($table_name, $id){
		if (!$this->existsID($table_name, $id)) return false;
		return $this->del($table_name, "`id`= '$id'");
	}
